...
async function stripeDeactivatePromotionCode(id, stripeKey) {
  ...
  const data = await res.json();
  if (!res.ok) throw new Error(data?.error?.message || 'Stripe could not deactivate the promotion code.');
  return data;
}
...
exports.handler = async function(event) {
  ...
  try {
    ...
    if (action === 'list_all') {
      const data = await callSheet('list_all', { limit: 250 });
      return response(200, { ok: true, reviews: data.reviews || [] });
    }
    ...
    if (action === 'test') {
      const data = await callSheet('mark_test', { review_id: reviewId, note: clean(body.note, 500) });
      return response(200, { ok: true, message: data.message || 'Review marked as Test.' });
    }

    if (action === 'delete') {
      const rewardData = await callSheet('get_review_reward', { review_id: reviewId });
      const rewards = Array.isArray(rewardData.rewards) ? rewardData.rewards : [];
      const promoIds = [...new Set(rewards.map(r => clean(r.stripe_promotion_code_id, 200)).filter(Boolean))];
      if (promoIds.length) {
        const stripeKey = process.env.STRIPE_SECRET_KEY;
        if (!stripeKey) return response(500, { error: 'Stripe key is required to deactivate the review promotion code before deleting this review.' });
        for (const id of promoIds) await stripeDeactivatePromotionCode(id, stripeKey);
      }
      const data = await callSheet('delete_review', { review_id: reviewId });
      return response(200, { ok: true, message: promoIds.length ? 'Review deleted and associated Stripe promotion code deactivated.' : (data.message || 'Review deleted.') });
    }
    ...
    if (String(review.status || '').toLowerCase() === 'test') return response(400, { error: 'Test reviews cannot be approved or receive coupons.' });
    ...
    if (rewardState.has_issued_reward) {
      ...
      return response(200, { ok: true, message: approved.message || 'Review approved without a new coupon.', email_sent: false, promo_code: rewardState.promo_code || '' });
    }
